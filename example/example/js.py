...
def encrypt_pwd(pwd):
    ctx = compile("""
    var rng_array = new Array(); 
    // 循环N次生成随机数 
    for(var i = 0 ; ; i++){ 
        // 只生成10个随机数 
        if(rng_array.length<32){ 
              generateRandom(240); 
        }else{ 
          break; 
       } 
    ...
    function generateRandom(count){ 
         ...
         for(var i = 0 ; i < rng_array.length; i++){ 
              if(rng_array[i] == rand){ 
                   return false; 
              }      
         } 
    ...
    function RSAEncrypt(a) {				// a是密码， b是加密过后的公钥, 128位
    ...
    if (null == rng_pool) {
        ...
        rng_pptr = 0;
        var t;
        for (rng_array[0], t = 0; 32 > t; ++t) rng_pool[rng_pptr++] = rng_array[t]
    ...
    ctx = compile(
        ...
        function encode(a) {
	        ...
	        do 
	            ...
	            h = a.charCodeAt(j++), 
	            d = b >> 2, 
	            e = (3 & b) << 4 | c >> 4, f = (15 & c) << 2 | h >> 6, 
	            i = 63 & h, isNaN(c) ? f = i = 64 : isNaN(h) && (i = 64), 
	            g = g + keyStr.charAt(d) + keyStr.charAt(e) + keyStr.charAt(f) + keyStr.charAt(i), 
	            b = c = h = "", d = e = f = i = "";
	        while (j < a.length);
	        return g
	        }
